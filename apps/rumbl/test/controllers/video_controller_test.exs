defmodule Rumbl.VideoControllerTest do
  ...
  @tag login_as: "max"
  test "does not create user video and renders error when invalid", %{conn: conn} do
    count_before = video_count(Video)
    conn = post conn, video_path(conn, :create), video: @invalid_attrs
    assert html_response(conn, 200) =~ ~r/Oops/
  ...
  test "authorizes actions against access by other users", %{conn: conn, user: owner} do
    ...
    assert_error_sent :not_found, fn ->
      put(conn, video_path(conn, :update, video.id, video: @valid_attrs))
    end
    assert_error_sent :not_found, fn ->
      get(conn, video_path(conn, :delete, video.id))
    end
  end
end
